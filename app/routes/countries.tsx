...
export default function Countries({ loaderData }: Route.ComponentProps) {
  ...
  // filtering / searching
  const filteredCountries = loaderData?.filter(
    (country: any) => (!search || country?.name?.common?.toLowerCase().includes(search?.toLowerCase())) && (!region || country?.region?.toLowerCase() === region?.toLowerCase())
  );

  // !note => elaborate version of above filtering function
  // const filtered = loaderData?.filter((country: any) => {
